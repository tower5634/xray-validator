...
def analyze_and_display(df, revenue_col, price_col, label="Product"):
    ...
    if "Product Details" in df.columns:
        first_product = str(df["Product Details"].iloc[0]).split()[0:6]  # first few words
        search_query = urllib.parse.quote_plus(" ".join(first_product))
        ...
        st.warning("⚠️ Could not find 'Product Details' column to generate Alibaba link.")
